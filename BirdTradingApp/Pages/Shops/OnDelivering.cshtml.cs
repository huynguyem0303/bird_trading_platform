...
{
    public class OnDeliveringModel : PageModel
    {
        private readonly IUnitOfWork _unitOfWork;
        public OnDeliveringModel(IUnitOfWork unitOfWork)
        {
        ...
        public async Task OnGetAsync(string searchString, string searchBy)
        {
            ...
            foreach (var item in OrderDetail)
            {
                ...
                {
                    ShippingSession = _unitOfWork.ShippingSessionRepository.GetByOrderDetailIdAndStatusAsync(item.Id, OrderStatus.OnDelelivering).Result;
                    continue;
                }
                if (ShippingSession != null)
                {
                    ShippingSession.AddRange(_unitOfWork.ShippingSessionRepository.GetByOrderDetailIdAndStatusAsync(item.Id, OrderStatus.OnDelelivering).Result);
                    continue;
                }
